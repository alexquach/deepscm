...
class BaseCovariateExperiment(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        split_dir = self.hparams.split_dir if hasattr(self.hparams, 'split_dir') else './'
        # data_dir = self.hparams.data_dir if hasattr(self.hparams, 'data_dir') else '/vol/biomedic2/bglocker/gemini/UKBB/t0/'

        self.celeba_train = CelebaEmbedDataset(npy_path=f'{split_dir}/train_features.npy', csv_path=f'{split_dir}/train_features.csv')  # noqa: E501
        self.celeba_val = CelebaEmbedDataset(npy_path=f'{split_dir}/val_features.npy', csv_path=f'{split_dir}/val_features.csv')
        self.celeba_test = CelebaEmbedDataset(npy_path=f'{split_dir}/test_features.npy', csv_path=f'{split_dir}/test_features.csv')
        ...
        # # tensor([[ 800000., 1100000., 1400000.,  800000., 1100000., 1400000.,  800000., 1100000., 1400000.]]) .T
        ...
        # self.pyro_model.age_flow_lognorm_loc = (self.celeba_train.metrics['age'].log().mean().to(self.torch_device).float())
        # self.pyro_model.age_flow_lognorm_scale = (self.celeba_train.metrics['age'].log().std().to(self.torch_device).float())

        # self.pyro_model.ventricle_volume_flow_lognorm_loc = (self.celeba_train.metrics['ventricle_volume'].log().mean().to(self.torch_device).float())
        # self.pyro_model.ventricle_volume_flow_lognorm_scale = (self.celeba_train.metrics['ventricle_volume'].log().std().to(self.torch_device).float())

        # self.pyro_model.brain_volume_flow_lognorm_loc = (self.celeba_train.metrics['brain_volume'].log().mean().to(self.torch_device).float())
        # self.pyro_model.brain_volume_flow_lognorm_scale = (self.celeba_train.metrics['brain_volume'].log().std().to(self.torch_device).float())
    ...
    def train_dataloader(self):
        return DataLoader(self.celeba_train, batch_size=self.train_batch_size, shuffle=True)

    def val_dataloader(self):
        self.val_loader = DataLoader(self.celeba_val, batch_size=self.test_batch_size, shuffle=False)
        return self.val_loader

    def test_dataloader(self):
        self.test_loader = DataLoader(self.celeba_test, batch_size=self.test_batch_size, shuffle=False)
    ...
    def test_epoch_end(self, outputs):
        ...
        samples['unconditional_samples'] = {
            ...
            'facial_hair': sample_trace.nodes['facial_hair']['value'].cpu(),
        ...
        cond_data = {'facial_hair': self.facial_hair_range, 'z': self.z_range}
        cond_data = {
            'facial_hair': self.facial_hair_range.repeat(self.hparams.test_batch_size, 1),
    ...
    def get_counterfactual_conditions(self, batch):
        counterfactuals = {
            'do(facial_hair=1)': {'facial_hair': torch.ones_like(batch['facial_hair']) * 1},
            'do(facial_hair=2)': {'facial_hair': torch.ones_like(batch['facial_hair']) * 2},
            'do(facial_hair=3)': {'facial_hair': torch.ones_like(batch['facial_hair']) * 3},
    ...
    def build_reconstruction(self, x, sex, facial_hair, tag='reconstruction'):
        obs = {'x': x, 'sex': sex, 'facial_hair': facial_hair}
        ...
        self.logger.experiment.add_scalar(f'{tag}/mse', torch.mean(torch.square(x - recon).sum((1))), self.current_epoch)
    ...
    def build_counterfactual(self, tag, obs, conditions, absolute=None):
        ...
        sampled_kdes = {'orig': {'facial_hair': obs['facial_hair']}}
    ...
    def sample_images(self):
        with torch.no_grad():
            ...
            sampled_facial_hair = sample_trace.nodes['facial_hair']['value']
            ...
            kde_data = {
                'batch': {'facial_hair': obs_batch['facial_hair']},
                'sampled': {'facial_hair': sampled_facial_hair}
            }
            # kde_data = {
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--split_dir', default="./assets/data/celeba", type=str, help="split dir (default: %(default)s)")  # noqa: E501
