...
from deepscm.arch.celeba import Decoder, Encoder
...
from deepscm.experiments.celeba.base_experiment import BaseCovariateExperiment, BaseSEM, EXPERIMENT_REGISTRY, MODEL_REGISTRY  # noqa: F401
...
class BaseVISEM(BaseSEM):
    ...
    def __init__(self, latent_dim: int, logstd_init: float = -5, enc_filters: str = '16,32,64,128', dec_filters: str = '128,64,32,16',
        ...
        # decoder parts
        decoder = Decoder(latent_dim=self.latent_dim + self.context_dim)

        if self.decoder_type == 'fixed_var':
            self.decoder = DeepIndepNormal(decoder, 384, 384)
    ...
    def _get_transformed_x_dist(self, latent):
        ...
        x_base_dist = Normal(self.x_base_loc, self.x_base_scale).to_event(1)
        ...
        if isinstance(x_pred_dist, MultivariateNormal) or isinstance(x_pred_dist, LowRankMultivariateNormal):
            ...
            x_reparam_transform = AffineTransform(x_pred_dist.loc, x_pred_dist.scale, 1)
    ...
    @pyro_method
    def svi_guide(self, x, sex, facial_hair):
        self.guide(x, sex, facial_hair)

    @pyro_method
    def svi_model(self, x, sex, facial_hair):
        with pyro.plate('observations', x.shape[0]):
            pyro.condition(self.model, data={'x': x, 'sex': sex, 'facial_hair': facial_hair})()
    ...
    @pyro_method
    def reconstruct(self, x, sex, facial_hair, num_particles: int = 1):
        ...
        for _ in range(num_particles):
            ...
            recon, *_ = pyro.poutine.condition(
                self.sample, data={'sex': sex, 'facial_hair': facial_hair, 'z': z})(x.shape[0])
            recons += [recon]
...
class SVIExperiment(BaseCovariateExperiment):
    ...
    def prep_batch(self, batch):
        ...
        x = batch['embed']
        sex = batch['Male'].unsqueeze(1).float()
        facial_hair = batch['Facial_Hair'].unsqueeze(1).float()

        return {'x': x, 'sex': sex, 'facial_hair': facial_hair}
